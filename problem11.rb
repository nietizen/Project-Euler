...
#http://sameshirteveryday.com/2007/06/09/wheres-mathmin-and-mathmax-in-ruby/
module Math
  def self.Max(a, b)
    a > b ? a : b
  end
...
#Just creating the array and putting the values into it
#20x20, in the proble description
a=Array.new(20,0)
...
#inefficient, but takes the array and transforms it from string to int
for i in (0 .. a.length-1)
  a[i].each_index { |x| a[i][x] = a[i][x].to_i }
end

#placeholder values for manually checking.
b = 0
c = 0
d = ''

#maximal values of each type
maximum = 0
across = 0
down = 0
diag = 0
...
#no memory stored from previously found answers.
for i in (0 ... a.length)
for j in (0 ... a[i].length)

  #from the current value going across.  If there are not enough, ignore.
  if (j + 3 < a[i].length)
    across = a[i][j] * a[i][j+1] * a[i][j+2] * a[i][j+3]
    if (across > maximum)
      b = i
      c = j
      d = 'across'
  ...
  #from the current value going down.  If there are not enough, ignore.
  if (i + 3 < a.length)
    ...
    maximum = Math.Max(maximum,down)
  end

  #from the current value diag, slope 1.  If there are not enough, ignore.
  if (j + 3 < a[i].length && i + 3 < a.length)
